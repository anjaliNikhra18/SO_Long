...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/05/16 20:29:40 by aneekhra          #+#    #+#             */
/*   Updated: 2024/05/17 15:28:29 by aneekhra         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_player(t_data *win)
{
	if (win->map[win->player_y][win->player_x + 1] != WALL)
		...
		win->img->player = mlx_xpm_file_to_image(win->mlx_connection, \
		"textures/Playerplayer_D.xpm", &win->img->img_w, &win->img->img_h);
}
...
void	calculate_moves(t_data *win)
{
	char	*moves;
	char	*s;
	char	*c;
	...
	free(c);
	mlx_string_put(win->mlx_connection, win->mlx_window, 10, \
					20, 0xFFFFFF, win->put_moves);
	mlx_string_put(win->mlx_connection, win->mlx_window, 10, \
					40, 0xFFFFFF, win->put_collect);
